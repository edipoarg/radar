...
import { parseTsvDateToJsDate, mapChomp, Classifier } from "../utils.cjs";

describe("parseTsvDateToJsDate", () => {
  it("parses DD/MM/YYYY dates correctly", () => {
    let d = parseTsvDateToJsDate("10/12/1999");
    expect(d).toEqual(new Date(1999, 11, 10));
  });

  ["10/12/19", "10/12/1999/2", "12/14/1999"].map((d) =>
    describe(`with ${d}`, () => {
      it(`errors on invalid date: ${d}`, () => {
        let D = parseTsvDateToJsDate(d);
        expect(D).toBe(null);
      });
    }),
  );
